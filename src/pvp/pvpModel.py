...
import pickle
import numpy as np
sys.path.append("/Users/kevinnguyen/Projects/tennisml/src/predict/")
...
parser.add_argument("--trainingFile", default="weighted_1990_2017_training_all.csv", help="File with training data")
...
if __name__ == "__main__":
	...
	print "Training base models on entire dataset"
	models = trainModels(data, labels)

	print "Training stacked model on entire dataset"
	clr = trainStackedModel(trainingDataset, trainLabels)

	print "Pickling models"
	# Pickling models for later use
	modelInfo = {}
	modelInfo["baseModels"] = models
	modelInfo["stackedModels"] = clr
	modelInfo["bestFeatureIndices"] = bestFeatureIndices
	f_out = open("models.p", "w")
	pickle.dump(modelInfo, f_out)
	f_out.close()